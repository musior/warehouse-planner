...
export function renderKPI(kpi) {
  ...
  el.innerHTML = [
    {
      label: "Auta w drodze",
      value: kpi.inboundTrucks,
      sub: `z ${kpi.totalTrucks} awizacji`,
      cls: "",
    },
    {
      label: "Auta przybyłe",
      value: kpi.arrivedTrucks,
      sub: "już na placu / rozładowane",
      cls: kpi.arrivedTrucks > 0 ? "ok" : "",
    },
    {
      label: "Palet hipotetycznie",
      value: kpi.totalPalletsFromSSCC,
      sub: "wg raportu SSCC Inbound",
      cls: kpi.totalPalletsFromSSCC > 300 ? "warn" : "",
    },
    {
      label: "Bez danych SSCC",
      value: kpi.noSsccTrucks,
      sub: "awizacje bez dopasowania",
      cls: kpi.noSsccTrucks > 0 ? "warn" : "",
    },
  ]
    .map(
      (c) => `
...
function buildTruckRow(t) {
  ...
  const destStr =
    t.destinations.length > 0 ? `${t.destinations.length} kier.` : "—";
  const celne = t.isCelne ? `<span class="badge badge-celne">CELNE</span>` : "";
  const kontener = t.isKontenerManual
    ? `<span class="badge badge-kontener-manual">KON.MAN</span>`
    : t.isKontener
      ? `<span class="badge badge-kontener">KON</span>`
      : "";
...
export function selectTruck(sis) {
  _selectedSis = sis;
  document
    .querySelectorAll(".truck-row")
    .forEach((el) => el.classList.toggle("selected", el.dataset.sis === sis));
  if (!_model) return;
  ...
  wrap.innerHTML = `
    <table class="data-table">
      ...
      <tbody>
        ${rows
          .map(
            (r) => `<tr${r.isKontener ? ' class="row-kontener"' : ""}>
      ...
      <tbody>
        ${trucks
          .map((t) => {
            ...
            const bxNP = t.rows.filter(
              (r) => r.packageTypeCode === "BX" && !r.parentSscc,
            ).length;
...
export function updateSlotUI(slotId, status, fileName) {
  ...
  if (iconEl) {
    iconEl.className = `slot-status-icon status-${status}`;
    iconEl.textContent =
      status === "ok"
        ? "✓"
        : status === "error"
...
export function renderProcessesTab(
  ...
  if (!staffing && trucks.length === 0) {
    wrap.innerHTML =
      '<div class="empty-state"><div class="empty-icon">&#9881;</div><div class="empty-text">Brak danych do obliczenia</div><div class="empty-sub">Wczytaj pliki Awizacje i SSCC Inbound</div></div>';
    return;
  }
  ...
  const crossFlowNote =
    ...
    "</strong> kart." +
    "&nbsp;&#8594;&nbsp; reko: <strong>" +
    staffing.crossPalletsReko +
    "</strong> pal. (÷10)" +
    "&nbsp;&#8594;&nbsp; folia: <strong>" +
    staffing.crossPalletsFolia +
    "</strong> pal. (×0.75)" +
    "</div>";
  ...
  wrap.innerHTML =
    ...
    (staffing.manualCartons > 0
      ? buildSummaryItem("Kart. Kont. Manual", staffing.manualCartons, false)
      : "") +
    (staffing.dgBoxes > 0
      ? buildSummaryItem("Kart. Sort. DG", staffing.dgBoxes, false)
      : "") +
    (staffing.crossBoxes > 0
      ? buildSummaryItem("Kart. Sort. CROSS", staffing.crossBoxes, false)
      : "") +
    ...
    buildProcessCard({
      ...
      tooltip: ttSimple(
        "Palety",
        "Suma palet ze wybranych transportów (BX, Awizacja)",
        "1,4838",
        275,
      ),
    }) +
    (manualContainer.unitCount > 0
      ? buildProcessCard({
          id: "manualContainer",
          icon: "&#128230;",
          label: manualContainer.label,
          result: manualContainer,
          color: "amber",
          tooltip: ttSimple(
            "Kartony",
            "Kont. Manual (SOS=KONTENER MANUAL, Celne=W drodze)",
            "0,3885",
            1050,
          ),
        })
    ...
    '<span class="process-section-total">' +
    (sortingDg.peopleExact +
      drobnical.peopleExact +
      wstawianiePalet.peopleExact) +
    " os.</span>" +
    "</div>" +
    '<div class="process-cards">' +
    buildProcessCard({
      id: "sortingDg",
      icon: "&#127968;",
      label: sortingDg.label,
      result: sortingDg,
      color: "green",
      tooltip: ttSimple(
        "Kartony",
        "BX: Customer=PL Dabrowa Hub, Shipper=3M + kont. Celne=W drodze",
        "0,3467",
        1177,
      ),
    }) +
    buildProcessCard({
      id: "drobnical",
      icon: "&#128269;",
      label: drobnical.label,
      result: drobnical,
      color: "green",
      extraRows: [
        {
          label: "ATII drobnica (count<20, vol<0.1)",
          value: staffing.dgDrobnicalItems,
        },
      ],
      tooltip: ttSimple(
        "Poz. ATII",
        "Unikalne ATII (count&lt;20 i vol&lt;0.1); Customer=DG/SOLVENTUM, Shipper=3M",
        "0,44",
        927,
      ),
    }) +
    buildProcessCard({
      id: "wstawianiePalet",
      icon: "&#128230;",
      label: wstawianiePalet.label,
      result: wstawianiePalet,
      color: "teal",
      extraRows: [
        {
          label: "Palety_z_20K (over01vol+over20)",
          value: staffing.dgPaletyZ20K,
        },
        { label: "Pelne palety DG (PE)", value: staffing.dgPelnePalety },
        { label: "Kontener ST", value: staffing.dgKontenerSt },
        {
          label: "FTE_20K + FTE_FP",
          value: wstawianiePalet.fte20K + " + " + wstawianiePalet.fteFP,
        },
      ],
      tooltip: ttTwoComp(
        "Palety_z_20K (over01vol+over20)",
        "Pełne palety DG (PE) + Kontener ST",
        "2,9073",
        140,
      ),
    }) +
    buildProcessCard({
      id: "przygotowaniePalet",
      icon: "&#128218;",
      label: przygotowaniePalet.label,
      result: przygotowaniePalet,
      color: "indigo",
      extraRows: [
        {
          label: "Palety_z_20K / " + przygotowaniePalet.bench20K,
          value: "FTE_20K = " + przygotowaniePalet.fte20K,
        },
        {
          label: "(PE+ST) / " + przygotowaniePalet.benchFP,
          value: "FTE_FP = " + przygotowaniePalet.fteFP,
        },
      ],
    ...
    '<span class="process-section-total">' +
    (sortingCross.peopleExact +
      recoCross.peopleExact +
      foliaCross.peopleExact) +
    " os.</span>" +
    "</div>" +
    ...
    buildProcessCard({
      id: "sortingCross",
      icon: "&#8635;",
      label: sortingCross.label,
      result: sortingCross,
      color: "purple",
      tooltip: ttSimple(
        "Kartony",
        "BX: Customer&ne;DG/SOLVENTUM, Shipper=3M, EffArrival puste",
        "0,3724",
        1096,
      ),
    }) +
    ...
    buildProcessCard({
      ...
      tooltip: ttSimple(
        "Palety",
        "(Kartony CROSS &divide; 10) &times; 0,75 = palety do folii",
        "3,49",
        117,
      ),
    }) +
...
function buildTransportSelectionTable(trucks, selectedSisSet) {
  ...
  const statusLabel = (s) =>
    s === "inbound" ? "W drodze" : s === "arrived" ? "Przybyły" : "Brak SSCC";
  const statusClass = (s) =>
    s === "inbound"
      ? "ts-badge--inbound"
      : s === "arrived"
  ...
  const rows = trucks
    .map((t) => {
      ...
      return (
        "<tr>" +
        '<td class="ts-cb-cell"><input type="checkbox" class="truck-select-cb" data-sis="' +
        t.sis +
        '" ' +
        checked +
        "></td>" +
        ...
        (t.godzTime || "—") +
        "</td>" +
        '<td><span class="ts-badge ' +
        statusClass(t.status) +
        '">' +
        statusLabel(t.status) +
        "</span></td>" +
        '<td class="ts-num">' +
        (t.pallets ? t.pallets.total : 0) +
        "</td>" +
        "</tr>"
      );
    })
  ...
  return (
    ...
    "<thead><tr>" +
    '<th class="ts-cb-cell"><input type="checkbox" class="truck-select-cb" data-all="1" ' +
    allChecked +
    "></th>" +
    "<th>SIS</th>" +
    "<th>Nr rej.</th>" +
    ...
    fields
      .map(function (f) {
        ...
        return (
          ...
          '<span class="card-tooltip-field-name">' +
          f.name +
          "</span>" +
          fmls
            .map(function (fml) {
              return (
                '<span class="card-tooltip-field-formula">' + fml + "</span>"
              );
            })
            .join("") +
          "</div>"
...
const _TT_UTIL =
  "FTE &divide; ceil(FTE) &times; 100% &nbsp;&nbsp; ≥90% zielony · ≥70% żółty · &lt;70% szary";
...
function ttSimple(unitCap, unitSrc, mpu, bench) {
  return makeTooltip([
    ...
    {
      name: unitCap + "/os./zmianę",
      formula: "480 &times; 85% &divide; " + mpu + " &asymp; " + bench,
    },
...
function buildProcessCard({
  ...
  const extraHtml = extraRows
    .map(
      (row) =>
        ...
        '<span class="process-breakdown-label">' +
        row.label +
        "</span>" +
        '<span class="process-breakdown-value">' +
        row.value +
        "</span>" +
        "</div>",
    )
  ...
  const tooltipHtml = tooltip
    ? '<div class="card-tooltip-wrap' +
      (tooltipFlip ? " card-tooltip-wrap--flip" : "") +
      (tooltipUp ? " card-tooltip-wrap--up" : "") +
      '">' +
    ...
    : "";
  return (
    '<div class="process-card process-card--' +
    color +
    '" id="process-card-' +
    id +
    '">' +
    ...
    "</div>" +
    '<div class="process-people-exact">ceil: ' +
    result.peopleCeil +
    " os.</div>" +
    "</div>" +
    ...
    unitCapLabel +
    ' (wejście)</span><span class="process-breakdown-value">' +
    result.unitCount +
    "</span></div>" +
    '<div class="process-breakdown-row"><span class="process-breakdown-label">Czas łączny</span><span class="process-breakdown-value">' +
    result.minutesNeeded +
    " min</span></div>" +
    '<div class="process-breakdown-row"><span class="process-breakdown-label">' +
    unitCapLabel +
    '/os./zmianę</span><span class="process-breakdown-value">~' +
    result.unitsPerPerson +
    "</span></div>" +
    '<div class="process-breakdown-row"><span class="process-breakdown-label">Wykorz. zmiany</span><span class="process-breakdown-value util-' +
    utilizationColor +
    '">' +
...
function buildProcessCardPlaceholder(label) {
  return (
    ...
    '<div class="process-card-header"><span class="process-card-icon">&#8943;</span><span class="process-card-label">' +
    label +
    "</span></div>" +
...
export function renderTimesTab() {
  ...
  for (const group of groups) {
    rows +=
      '<tr class="times-group-header"><td colspan="3">' +
      esc(group.title) +
      "</td></tr>";
